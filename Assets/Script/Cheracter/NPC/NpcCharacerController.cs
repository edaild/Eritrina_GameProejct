...
public class NpcCharacterController : MonoBehaviour
{
    ...
    public GameObject mainMenu; // 메인 메뉴 (현재 비활성화)
    public int text_Count = 0; // 대화 카운트

    private bool mouseClick; // 마우스 클릭 여부 (현재 사용되지 않음)
    private float cs_time; // 시간 변수 (현재 사용되지 않음)
    ...
    private void Start()
    {
        ...
        textUI.SetActive(false); // 대화 UI 비활성화
                                 //   FKeyUI.SetActive(false); // F 키 안내 UI 비활성화
    }
